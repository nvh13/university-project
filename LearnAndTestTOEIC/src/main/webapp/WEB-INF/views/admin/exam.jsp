                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tr>
                                        ...
                                        <th>Tên</th>
                                        <th>Ảnh</th>
                                        <th><button type="button" class='btn btn-block btn-success btn-sm' onclick="redirectAddPage()"><i class="fa fa-plus-square"></i>&nbsp; Thêm bài thi</button></th>
                                        <th>Cập nhật</th>
                                        <th>Xóa</th>
                                    </tr>
                                    </thead>
...
<script>
    ...
    function getExam(page){
        $.ajax({
            url: '${pageContext.request.contextPath}/api/v1/exam?page='+ page +'&per_page=5',
            type: 'GET',
            dataType: 'JSON',
            success: function (result) {
                ...
                $.each(result.data, function (key ,value) {
                    $('#examBody').append(
                        ...
                        "<td>" + value.name + "</td> " +
                        "<td>" + value.image + "</td> " +
                        "<td> <a href='<c:url value='/admin/examQuestion?id=" + value.id + "'/>'><button type='button' class='btn btn-block btn-info btn-sm'>Chi tiết</button></a></td>" +
    ...
    function redirectAddPage(){
        window.location.href = "${pageContext.request.contextPath}/admin/exam/created";
    }
    function redirectUpdatePage(id){
        window.location.href = "${pageContext.request.contextPath}/admin/exam/updated?id="+id;
    }
    ...
    function deleteExam(id) {
        $.ajax({
            url: '${pageContext.request.contextPath}/api/v1/exam',
            type: 'DELETE',
            contentType: 'application/json',
            data: JSON.stringify(id),
            success: function (result) {
                window.location.href = "${pageContext.request.contextPath}/admin/exam?code="+result.code+"&message="+result.message;
            }
        });
    }
</script>
